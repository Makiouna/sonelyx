...
export function groupQuotesByProject(quotes: Quote[]): ProjectGroup[] {
  ...
  // Build groups with auto-names (oldest-first at this point)
  const intermediate = rawGroups.map(qs => {
    const startDates = qs.map(q => q.startDate).sort();
    const endDates   = qs.map(q => q.endDate).sort();
    return {
      id: qs[0].id,
      autoName: autoGroupName(startDates[0]),
      customName: qs[0].projectName ?? null,
      quotes: qs,
  ...
  // Count how many groups share the same auto-name (only among those without a custom name)
  ...
  // Assign suffix (1), (2)… in chronological order (oldest first = (1))
  ...
  const groups: ProjectGroup[] = intermediate.map(g => {
    ...
    const hasDupes = autoNameCount[g.autoName] > 1;
    if (hasDupes) {
...
export function projectStatusColors(status: ProjectStatus): { bg: string; color: string } {
  switch (status) {
    ...
    case 'validated':       return { bg: '#e2fbe8', color: '#1db954' };
