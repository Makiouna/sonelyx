...
export async function POST(request: Request) {
  try {
    ...
    const { quoteId, amountToCapture } = await request.json();
    if (!quoteId) {
    ...
    // Partial capture if amountToCapture is specified and less than depositAmount
    if (amountToCapture && amountToCapture > 0 && amountToCapture < (quote.depositAmount ?? 0)) {
    ...
    return NextResponse.json({ success: true, message: 'Caution encaissée avec succès.' });
  } catch (error: any) {
